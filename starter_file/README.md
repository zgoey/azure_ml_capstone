...
A better model may be found if we allow AutoML to run longer than 1 hour. Actually, our hyperparameter search ran much longer than that, so it would only be fair to give AutoML a little bit more space in this respect. However, since the accuracies found appear to lie within a pretty narrow bandwidth, we do not really expect any huge improvements.
